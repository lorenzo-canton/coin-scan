...
def get_data(connection, sym, per, num):
    return connection.get_candles(sym, period=per, number=num)


def thread_function(name, connection, data):
    con.logger.disabled = True
    while True:
        logging.info("Thread %s", name)
        print(get_data(connection))
        time.sleep(timeframe[data['tf']] * 60)
...
if __name__ == "__main__":
    ...
    for i in traders:
        x = threading.Thread(target=i['target'], args=(i['name'], con, i['data'],))
        x.start()
